...
loadProgramToMemory input = Map.fromList (List.zip [0 ..] input)
...
parseToAction x = getAction $ getModes
  where
    ...
    getAction (m1, m2, m3)
      | d == 9 && e == 9 = Halt
      | d == 0 && e == 9 = AdjustRealtive m1
      | d == 0 && e == 8 = Equals m1 m2 m3
      | d == 0 && e == 7 = LessThan m1 m2 m3
      | d == 0 && e == 6 = JumpFalse m1 m2
      | d == 0 && e == 5 = JumpTrue m1 m2
      | d == 0 && e == 4 = Output m1
      | d == 0 && e == 3 = Input m1
      | d == 0 && e == 2 = Multiply m1 m2 m3
      | d == 0 && e == 1 = Add m1 m2 m3
...
programStep :: State -> Maybe Int -> (Effect, State)
programStep (Just cursor, memory, relativeOffset) input
  = case action of
    (Halt)              -> (Halted, (Nothing, memory, relativeOffset))
    (Output m1)         -> (Printed $ arg 1 m1, (Just (cursor + 2), memory, relativeOffset))
    (Input m1)          -> (InputGiven, (Just (cursor + 2), Map.insert (argWrite 1 m1) (fromJust input) memory, relativeOffset))
    (Add m1 m2 m3)      -> (InnerAction, (Just (cursor + 4), (Map.insert (argWrite 3 m3) ((arg 1 m1) + (arg 2 m2)) memory), relativeOffset))
    (Multiply m1 m2 m3) -> (InnerAction, (Just (cursor + 4), (Map.insert (argWrite 3 m3) ((arg 1 m1) * (arg 2 m2)) memory), relativeOffset))
    (JumpTrue m1 m2)    -> if arg 1 m1 /= 0 then  (InnerAction, (Just (arg 2 m2), memory, relativeOffset)) else (InnerAction, (Just (cursor + 3), memory, relativeOffset))
    (JumpFalse m1 m2)   -> if (arg 1 m1) == 0 then (InnerAction, (Just (arg 2 m2), memory, relativeOffset)) else (InnerAction, (Just (cursor + 3), memory, relativeOffset))
    (LessThan m1 m2 m3) -> if (arg 1 m1) < (arg 2 m2) then (InnerAction, (Just (cursor + 4), (Map.insert (argWrite 3 m3) 1 memory), relativeOffset)) else (InnerAction, (Just (cursor + 4), (Map.insert (argWrite 3 m3) 0 memory), relativeOffset))
    (Equals m1 m2 m3)   -> if (arg 1 m1) == (arg 2 m2) then (InnerAction, (Just (cursor + 4), (Map.insert (argWrite 3 m3) 1 memory), relativeOffset)) else (InnerAction, (Just (cursor + 4), (Map.insert (argWrite 3 m3) 0 memory), relativeOffset))
    (AdjustRealtive m1) -> (InnerAction, (Just (cursor + 2), memory, relativeOffset + (arg 1 m1)))
  where
...
run :: State -> [Int] -> [Int] -> ([Int], State)
run state [] output = case effect of
  (Printed x) -> run state' [] (output ++ [x])
  InnerAction -> run state' [] output
  InputGiven -> (output, state)
  where (effect, state') = programStep state (listToMaybe input)
run state input output = case effect of
  (Printed x) -> run state' input (output ++ [x])
  InnerAction -> run state' input output
  InputGiven -> run state' (tail input) output
  where (effect, state') = programStep state (listToMaybe input)
...
destination (Packet destination _ _) = destination
...
outputToPackets :: [Int] -> [Packet]
outputToPackets output = List.map (\[d, x, y] -> Packet d x y) . Split.chunksOf 3 $ output

packetsToInputs :: [Packet] -> [Int]
packetsToInputs packets = concat . List.map (\(Packet _ x y) -> [x, y]) $ packets
--------------------------------------------------------------------------------------

part1 :: Int
part1 = do
  let prog = (Just 0, loadProgramToMemory input, 0)
      machines = Map.fromList [(i, (prog, [i])) | i <- [0..49]]
  runNetwork machines
  where
    runNetwork :: Map.Map Int (State, [Int]) -> Int
    runNetwork machines =
      let 
        afterTick = Map.map (\(state, input) -> let (output, state') = run state input [] in (state', outputToPackets output)) machines
        packetsSent = Map.foldl (\acc (_, outputs) -> List.foldl (\acc p -> Map.insertWith (++) (destination p) [p] acc) acc outputs) Map.empty afterTick
        specialPacket = packetsSent !? 255
        machines' = Map.mapWithKey (\destination (prog', _) ->
                                      case Map.lookup destination packetsSent of
                                        Nothing -> (prog', [-1])
                                        Just ps -> (prog', packetsToInputs ps)
                                     ) afterTick
      in if isNothing specialPacket then runNetwork machines' else let ((Packet _ x y):ps) = fromJust specialPacket in y

part2 :: Int
part2 = do
  let prog = (Just 0, loadProgramToMemory input, 0)
      machines = Map.fromList [(i, (prog, [i])) | i <- [0..49]]
  loop machines Nothing [Packet (-1) (-1) (-1), Packet (-2) (-2) (-2)]

  where
    loop :: Map.Map Int (State, [Int]) -> Maybe Packet -> [Packet] -> Int
    loop machines nat natSent = do
      let machines' = Map.map (\(prog, inputs) ->
                               let (outputs, prog') = run prog inputs []
                               in (prog', outputToPackets outputs)
                               ) machines
      let packetsSent = Map.foldl (\acc (_, outputs) -> List.foldl (\acc p -> Map.insertWith (\n o -> o ++ n) (destination p) [p] acc) acc outputs) Map.empty machines'

      let nat' = case packetsSent !? 255 of
                   Nothing -> nat
                   Just packets -> Just (last packets)
              
      let (nextIter, natSent') =
            let plannedSend = Map.mapWithKey (\a (prog', _) ->
                                                case packetsSent !? a of
                                                  Nothing -> (prog', [-1])
                                                  Just ps -> (prog', packetsToInputs ps)
                                               ) machines'
            in  if Map.null packetsSent && isJust nat'
                then
                  (Map.insert 0 (let Just (prog', [-1]) = plannedSend !? 0
                                     Just (Packet _ x y) = nat'
                                 in  (prog', [x, y])) plannedSend, (fromJust nat') : natSent)
                else (plannedSend, natSent)           
      
      let ((Packet _ x0 y0):(Packet _ x1 y1):_) = natSent'
      if y0 == y1 then y0 else loop nextIter nat' natSent'
